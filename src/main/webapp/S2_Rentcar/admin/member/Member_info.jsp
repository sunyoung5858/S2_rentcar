...
<%
		response.sendRedirect("login.jsp");//메인페이지로 이동
	}else if(session.getAttribute("auth").equals("일반사용자")){
%>
	<script>
	alert('관리자 계정으로 로그인해주세요.')//메인페이지로 이동
	</script>
<% 
		response.sendRedirect("login.jsp");//지우님이 만든 로그인페이지jsp링크 넣기
